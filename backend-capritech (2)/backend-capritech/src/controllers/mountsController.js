const { mountCreate } = require("../services/mountsService");
const Response = require("../functions/response");

const getAllMounts = (req, res) => {
    const response = new Response(
        "Obteniendo todas las montas",
        { total: 10 },
        false
    );
...
const getMountById = (req, res) => {
    ...
    if (!id || id.trim() === "") {
        const response = new Response(
            ...
            null,
            "El ID es obligatorio para obtener una monta"
        );
...
const createMount = async (req, res) => {
    ...
    if (errores.length > 0) {
        var response = new Response(
            "Error en la creación de la monta",
            null,
            errores
        );
        res.status(400);
...
const updateMount = (req, res) => {
    const { id } = req.params;

    if (!id || id.trim() === "") {
        const response = new Response(
            "error al actualizar la monta",
            null,
            "El ID es obligatorio para actualizar una monta"
        );
        return res.status(400).json(response.json);
    }
...
const deleteMount = (req, res) => {
    const { id } = req.params;

    if (!id || id.trim() === "") {
        const response = new Response(
            "error al eliminar la monta",
            null,
            "El ID es obligatorio para eliminar una monta"
        );
        return res.status(400).json(response.json);
    }
